...
class AIClients(BaseClientManager):
    ...
    # 普通模式与 JSON 模式必须分开缓存：
    # response_format 只在实例创建时生效，共用一个槽位会让先创建的
    # JSON 模式实例被后续所有 response_format=False 的调用方误用
    _openai_llm: Optional[ChatOpenAI] = None
    _openai_llm_lock = threading.Lock()

    _openai_llm_json: Optional[ChatOpenAI] = None
    ...
    def get_openai_llm(cls, response_format=False) -> ChatOpenAI:
        # JSON 模式与普通模式各走自己的缓存槽，互不干扰
        if response_format:
            return cls._get_or_create(
                "_openai_llm_json", cls._openai_llm_json_lock,
                lambda: cls._create_openai_llm(True)
            )
        return cls._get_or_create(
            "_openai_llm", cls._openai_llm_lock,
            lambda: cls._create_openai_llm(False)
        )
    ...
    def _create_openai_llm(cls, response_format=False) -> ChatOpenAI:
        try:
            ...
            logger.info(
                f"ChatOpenAI LLM 客户端初始化成功 "
                f"(model={model_name}, base_url={base_url}, json_mode={response_format})"
            )
            return client
